...
- **Endpoints**:
  - `POST /api/sponsor` — Sponsor any frontend-built transaction kind
  - `POST /api/execute-transaction` — Execute sponsored transactions
  - `POST /api/walrus/access` — Token-gated Walrus object access
...
- **Manual EnokiFlow zkLogin integration** (Google OAuth + callback handling)
...
# Optional token-gating for Walrus access
WALRUS_SUBSCRIPTION_TYPE=0xYOUR_PACKAGE::subscription::SubscriptionToken
WALRUS_OBJECT_IDS=0xabc...,0xdef...
```
...
# Google redirect URI (must be whitelisted in Google Cloud OAuth client)
...
```json
{
  ...
  "allowedMoveCallTargets": ["0x...::hero::create_hero"],
  "allowedAddresses": ["0x..."]
}
```
