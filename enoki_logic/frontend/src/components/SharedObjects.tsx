...
import { buildBuyHeroTransaction } from "../utils/transactions";
...
export default function SharedObjects({ refreshKey, setRefreshKey }: RefreshProps) {
  ...
  const handleBuy = async (listHeroId: string, priceInSui: number) => {
    if (!accountAddress || !packageId) return;
    
    setIsBuying(prev => ({ ...prev, [listHeroId]: true }));
    
    try {
      // 1. Fetch user's SUI coins
      console.log("Fetching SUI coins for address:", accountAddress);
      const suiCoins = await fetchSuiCoins(suiClient, accountAddress);
      ...
      const tx = buildBuyHeroTransaction(
        packageId,
      ...
      {listedHeroes.length === 0 ? (
        ...
        <Grid columns="3" gap="4">
          {listedHeroes.map((obj) => {
            ...
            const heroFields = fields.hero.fields;
              ...
              <Card key={listHeroId} style={{ padding: "16px" }}>
                <Flex direction="column" gap="3">
                  ...
                  <Button 
                    ...
                    disabled={!accountAddress || isBuying[listHeroId] || isSigning}
